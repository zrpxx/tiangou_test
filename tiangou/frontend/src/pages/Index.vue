...
export default {
  ...
  mounted() {
    let _this = this
    this.$axios.get('https://tiangou.zrp.cool/api/user/info/' + this.$store.state.userid).then(function (response) {
    ...
    this.$axios.get('https://tiangou.zrp.cool/api/random').then(function (response) {
      let res = response.data
      if(res.status === 'Success') {
